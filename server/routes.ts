...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.mimetype.startsWith('audio/') || file.mimetype === 'audio/wav' || file.mimetype === 'audio/mp4') {
      cb(null, true);
    } else {
        ...
        try {
          ...
          if (!bucketExists) {
            ...
            const { data: newBucket, error: createError } = await supabase.storage.createBucket('audio-recordings', {
              public: false,
              allowedMimeTypes: ['audio/wav', 'audio/mp4', 'audio/mp3', 'audio/webm', 'audio/mpeg', 'audio/ogg'],
